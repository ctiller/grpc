...
grpc_channelz_v2_PropertyValue* PropertyValueFromValue(T value,
                                                       upb_Arena* arena) {
  auto* p = grpc_channelz_v2_PropertyValue_new(arena);
...
class PropertyTable {
 ...
 private:
  void SetValue(absl::string_view column, absl::string_view row,
                grpc_channelz_v2_PropertyValue* value);
  int LookupColumn(absl::string_view column);
  int LookupRow(absl::string_view row);
  void SetIndex(int column, int row, grpc_channelz_v2_PropertyValue* value);
 ...
 public:
  ...
  void Add(absl::string_view key, T value) {
    grpc_channelz_v2_PropertyList_values_set(
        ...
        property_list_detail::PropertyValueFromValue(std::move(value)));
  }
