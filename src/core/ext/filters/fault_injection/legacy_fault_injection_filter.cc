...
class LegacyFaultInjectionFilter::InjectionDecision {
 public:
...
absl::StatusOr<LegacyFaultInjectionFilter> LegacyFaultInjectionFilter::Create(
    const ChannelArgs&, ChannelFilter::Args filter_args) {
  return LegacyFaultInjectionFilter(filter_args);
}

LegacyFaultInjectionFilter::LegacyFaultInjectionFilter(
...
ArenaPromise<ServerMetadataHandle> LegacyFaultInjectionFilter::MakeCallPromise(
...
LegacyFaultInjectionFilter::InjectionDecision
LegacyFaultInjectionFilter::MakeInjectionDecision(
...
bool LegacyFaultInjectionFilter::InjectionDecision::HaveActiveFaultsQuota()
    const {
  return g_active_faults.load(std::memory_order_acquire) < max_faults_;
}

Timestamp LegacyFaultInjectionFilter::InjectionDecision::DelayUntil() {
...
absl::Status LegacyFaultInjectionFilter::InjectionDecision::MaybeAbort() const {
...
std::string LegacyFaultInjectionFilter::InjectionDecision::ToString() const {
  return absl::StrCat("delay=", delay_time_ != Duration::Zero(),
                      " abort=", abort_request_.has_value());
}

const grpc_channel_filter LegacyFaultInjectionFilter::kFilter =
    MakePromiseBasedFilter<LegacyFaultInjectionFilter, FilterEndpoint::kClient>(
