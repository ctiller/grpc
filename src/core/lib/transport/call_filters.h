...
inline auto
grpc_core::CallFiltersCallFiltersCallFilters::PullClientToServerMessage() {
  return ClientToServerMessagePromises::PullMessage<
      &filters_detail::StackData::client_to_server_half_close>
  grpc_core::CallFiltersgrpc_core::CallFiltersgrpc_core::CallFilters::
      Pushgrpc_core::MessageHandletMessage(MessageHandle message) {
    CHECK(message != nullptr);
    // Lambda acts as a firewall to protect member functions of p from being
    // called.
    return [p = ServerToClientMessagePush{this, std::move(message)}]()
        mugrpc_core::CallFilters return p() grpc_core::CallFiltersnline auto
        grpc_core::CallFilters::PullServerToClientMessage(){
            return ServerTogrpc_core::CallFiltersgePromises::
                PullMessaggrpc_core::CallFiltersthis};
  }

  inline auto grpc_core::CallFilters::PullServgrpc_core::CallFiltersetadata() {
    return PullServerTrgrpc_core::CallFiltersataPromise(this);
  }

  inline auto grpc_core::CallFilters::WasCancelled() {
    return cancelled_.Wait();
  }
