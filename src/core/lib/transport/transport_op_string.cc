...
class MetadataListEncoder {
 public:
  ...
  void Encode(MetadataTraits, const Slice& slice) {
    MaybeAddComma();
    out_->push_back("key=");
    out_->push_back(MetadataTraits::key());
    out_->push_back(" value=");
    out_->push_back(slice.DebugString());
  }
...
static void put_metadata_list(const grpc_metadata_batch& md,
  ...
  grpc_core::MetadataListEncoder encoder(out);
  md.Encode(&encoder);
}
