...
TEST(SubchannelStreamLimiterTest, Basic) {
  ...
  // Initial state: max_concurrent_streams = 0 (default in implementation
  // details, but SetMaxConcurrentStreams usually called first) Wait, default
  // constructor initializes stream_counts_ to 0, which means max=0,
  // in_flight=0.
  ...
  // Return quota
  EXPECT_TRUE(limiter.ReturnQuotaForRpc());  // Returns true because we are now
                                             // below quota (0 < 1)?
  // Wait, ReturnQuotaForRpc returns true if GetRpcsInFlight ==
  // GetMaxConcurrentStreams. If we had 1 in flight and max 1. After return, 0
  // in flight, max 1. 0 != 1. So it should return false? Let's check
  // implementation: return GetRpcsInFlight(prev) ==
  // GetMaxConcurrentStreams(prev); prev was 1 in flight, 1 max. So 1 == 1.
  // Returns true. This means "was full before return".
