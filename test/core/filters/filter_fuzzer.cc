...
class MainLoop {
 public:
  explicit MainLoop(std::unique_ptr<ChannelFilter> filter)
  ...
  auto activity = MakeActivity(
      [&filter]() {
        return (*filter)->MakeCallPromise(CallArgs{nullptr, nullptr},
                                          [](CallArgs) { abort(); })
      },
      ExecCtxWakeupScheduler(), [](ServerMetadataHandle) {});
*/
  }
