...
void CanSendFrames(size_t num_data_endpoints, uint32_t client_alignment,
  ...
  MpscReceiver<Frame> client_reciever(client_max_buffer_hint);
  MpscReceiver<Frame> server_reciever(server_max_buffer_hint);
  auto client_sender = client_reciever.MakeSender();
  auto server_sender = server_reciever.MakeSender();
  auto client_sink =
      MakeRefCounted<TestSink>(send_on_server_frames, engine.get());
  auto server_sink =
      MakeRefCounted<TestSink>(send_on_client_frames, engine.get());
  client_transport->Start(client_party.get(), std::move(client_reciever),
                          client_sink);
  server_transport->Start(server_party.get(), std::move(server_reciever),
                          server_sink);
