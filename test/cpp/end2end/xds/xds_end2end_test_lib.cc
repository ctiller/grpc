...
void XdsEnd2endTest::TearDown() {
  ...
  grpc_core::EnvSet("GRPC_XDS_BOOTSTRAP", absl::nullopt);
  grpc_core::EnvSet("GRPC_XDS_BOOTSTRAP_CONFIG", absl::nullopt);
...
void XdsEnd2endTest::InitClient(BootstrapBuilder builder,
  ...
  if (GetParam().bootstrap_source() == XdsTestType::kBootstrapFromEnvVar) {
    grpc_core::EnvSet("GRPC_XDS_BOOTSTRAP_CONFIG", bootstrap_.c_str());
    ...
    fclose(out);
    grpc_core::EnvSet("GRPC_XDS_BOOTSTRAP", bootstrap_file_);
  }
