...
def fix(filename):
  if filename.startswith('src/core/ext/upbdefs-generated'): return
  if filename.startswith('src/core/ext/upb-generated'): return
  if filename in [
  ...
  for line in text.splitlines():
    ...
    if state == "includes":
      if re.search(r"^\s*//.*", line):
        ...
        if m:
          includes.append((m.group(1), "project", comment, m.group(2)))
        m = re.search(r'<([^>]*)>(.*)', line)
        if m:
    ...
    if state == "postamble":
      if line.startswith("#include"):
        if not warned_about_skipping:
          print("WARNING: includes found after main block in %s: skipping formatting" % filename)
  ...
  for include in includes:
    if include[1] == "project" and include[0].startswith('include/grpc'):
      include = (include[0][len('include/'):], "system", include[2], include[3])
  ...
  for include in includes:
    if include[1] == "system" and include[0] == 'grpc/support/port_platform.h':
      has_port_platform = True
      continue
    if include[1] == "system" and include[0] == 'grpc/impl/codegen/port_platform.h':
      has_port_platform = True
      continue
    new_includes.append(include)
  ...
  first = True
  for include in includes:
    if include[1] == "system" and os.path.splitext(include[0])[1] != '' and not include[0].startswith('grpc'):
      if first:
        out += '\n'
        first = False
      for line in include[2]: out += '%s\n' % line
      out += '#include <%s>%s\n' % (include[0], include[3])
    else:
      new_includes.append(include)
  includes = new_includes
  ...
  for include in includes:
    if (include[1] == "project"
        and not include[0].startswith('src/')
        and not include[0].startswith('test/')
        and not include[0].startswith('grpc/')
        and not include[0].startswith('grpcpp/')):
      if first:
        out += '\n'
        first = False
      for line in include[2]: out += '%s\n' % line
